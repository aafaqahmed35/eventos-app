...
async function getBuildId(username) {
  try {
    ...
    const match = mainPage.data.match(
...
const fetchNewProfileData = async (username) => {
  try {
    ...
    if (problemNavbar.length === 0) {
      throw new Error(
        `User ${username} does not exist or has not solved any problems on GeeksForGeeks.`
      );
    }
    ...
    const values = {
      ...
      pod_solved_global_longest_streak:
        userInfo.pod_solved_global_longest_streak || 0,
    ...
    console.error(
      `Error fetching new profile data for ${username}: ${error.message}`
    );
    return null;
  }
};
...
const fetchDirectProfileData = async (username) => {
  try {
    ...
    let response = await axios.get(
      `https://www.geeksforgeeks.org/gfg-assets/_next/data/${buildId}/user/${username}.json`
    );
    ...
    console.error(
      `Error fetching profile data using api for ${username}: ${error.message}`
    );
    return null;
  }
};
